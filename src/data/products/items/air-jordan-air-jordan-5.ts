import type { Product } from '@/types';

export const AirJordanAirJordan5Product: Product = {
  id: 'product-004',
  slug: 'air-jordan-air-jordan-5',
  name: 'Air Jordan 5',
  brand: 'air-jordan',
  category: 'shoes',
  description: 'Air Jordan 5 is presented as part of the current shoe catalog, combining recognizable branding, wearable comfort and standout styling for everyday fashion use.',
  flylinkUrl: 'https://store.flylinking.com/g/M0OLHBPQF5',
  variants: [
    {
      color: 'Black',
      images: [
        '/images/products/air-jordan-air-jordan-5/Black1.webp',
        '/images/products/air-jordan-air-jordan-5/Black2.webp',
        '/images/products/air-jordan-air-jordan-5/Black3.webp',
        '/images/products/air-jordan-air-jordan-5/Black4.webp'
      ]
    },
    {
      color: 'Black Blue',
      images: [
        '/images/products/air-jordan-air-jordan-5/Black-Blue1.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Blue2.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Blue3.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Blue4.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Blue5.webp'
      ]
    },
    {
      color: 'Black Red',
      images: [
        '/images/products/air-jordan-air-jordan-5/Black-Red1.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red2.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red3.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red4.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red5.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red6.webp',
        '/images/products/air-jordan-air-jordan-5/Black-Red7.webp'
      ]
    }
  ],
  images: [
    '/images/products/air-jordan-air-jordan-5/Black1.webp',
    '/images/products/air-jordan-air-jordan-5/Black-Blue1.webp',
    '/images/products/air-jordan-air-jordan-5/Black-Red1.webp'
  ],
  colors: [
    'Black',
    'Black Blue',
    'Black Red'
  ],
  sizes: [
    'Men US 4 = Women US 5.5 = UK 3.5 = EUR 36',
    'Men US 4.5 = Women US 6 = UK 4 = EUR 36.5',
    'Men US 5 = Women US 6.5 = UK 4.5 = EUR 37.5',
    'Men US 5.5 = Women US 7 = UK 5 = EUR 38',
    'Men US 6.5 = Women US 8 = UK 6 = EUR 39',
    'Men US 7 = Women US 8.5 = UK 6.5 = EUR 40',
    'Men US 8 = Women US 9.5 = UK 7.5 = EUR 41',
    'Men US 8.5 = Women US 10 = UK 8 = EUR 42',
    'Men US 9.5 = Women US 11 = UK 9 = EUR 43',
    'Men US 10 = Women US 11.5 = UK 9.5 = EUR 44',
    'Men US 11 = Women US 12.5 = UK 10.5 = EUR 45',
    'Men US 12 = Women US 14 = UK 11 = EUR 46',
    'Men US 12.5 = Women US 14.5 = UK 11.5 = EUR 47'
  ],
  featured: true,
  newArrival: true,
  priceLabel: '$79.99',
  materials: [
    'Leather upper',
    'Rubber outsole',
    'Signature branded details'
  ]
};
