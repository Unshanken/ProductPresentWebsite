import type { Product } from '@/types';

export const AlexanderMcqueenOversizedLeatherSneakerProduct: Product = {
  id: 'product-021',
  slug: 'alexander-mcqueen-oversized-leather-sneaker',
  name: 'Oversized Leather Sneaker',
  brand: 'alexander-mcqueen',
  category: 'shoes',
  description: 'timeless, versatile take on the iconic McQueen oversized sneaker, featuring the signature clean white upper with a distinctive haze blue heel accent.\nCrafted from specially treated soft calfskin with anti-crease and stain-resistant properties, ensuring long-lasting structure and a refined look.\nLined with a premium lambskin blend for enhanced comfort and breathability.\nBuilt with a custom-molded TPU + PU double-layer sole, engineered to resist yellowing while delivering excellent rebound and cushioning.\nThe dual-density sole construction provides superior comfort, making it suitable for daily wear, light sports, and outdoor activities.\nA perfect balance of streetwear edge and modern luxury styling.',
  flylinkUrl: 'https://store.flylinking.com/g/FOULX0A5AH',
  variants: [
    {
      color: 'Blue',
      images: [
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Blue1.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Blue2.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Blue3.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Blue4.jpg'
      ]
    },
    {
      color: 'Green',
      images: [
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green1.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green2.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green3.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green4.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green5.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green6.jpg'
      ]
    },
    {
      color: 'Pink',
      images: [
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Pink1.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Pink2.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Pink3.jpg',
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/Pink4.jpg'
      ]
    },
    {
      color: 'More Color On Flylink',
      images: [
        '/images/products/alexander-mcqueen-oversized-leather-sneaker/More Color on Flylink.jpg'
      ]
    }
  ],
  images: [
    '/images/products/alexander-mcqueen-oversized-leather-sneaker/Blue1.jpg',
    '/images/products/alexander-mcqueen-oversized-leather-sneaker/Green1.jpg',
    '/images/products/alexander-mcqueen-oversized-leather-sneaker/Pink1.jpg',
    '/images/products/alexander-mcqueen-oversized-leather-sneaker/More Color on Flylink.jpg'
  ],
  colors: [
    'Blue',
    'Green',
    'Pink',
    'More Color On Flylink'
  ],
  sizes: [
    'Men US 3.5 = Women US 5 = UK 3 = EUR 35.5',
    'Men US 4 = Women US 5.5 = UK 3.5 = EUR 36',
    'Men US 4.5 = Women US 6 = UK 4 = EUR 36.5',
    'Men US 5 = Women US 6.5 = UK 4.5 = EUR 37.5',
    'Men US 5.5 = Women US 7 = UK 5 = EUR 38',
    'Men US 6.5 = Women US 8 = UK 6 = EUR 39',
    'Men US 7 = Women US 8.5 = UK 6.5 = EUR 40',
    'Men US 8 = Women US 9.5 = UK 7.5 = EUR 41',
    'Men US 8.5 = Women US 10 = UK 8 = EUR 42',
    'Men US 9.5 = Women US 11 = UK 9 = EUR 43',
    'Men US 10 = Women US 11.5 = UK 9.5 = EUR 44',
    'Men US 11 = Women US 12.5 = UK 10.5 = EUR 45'
  ],
  featured: false,
  newArrival: false,
  priceLabel: '$99.99',
  materials: [
  'Soft calfskin leather upper (anti-crease, stain-resistant)',
  'Lambskin blend lining',
  'TPU + PU double-layer sole',
  'Cushioned dual-density midsole',
  'Rubber outsole',
  'Signature heel tab detail'
  ]
};
